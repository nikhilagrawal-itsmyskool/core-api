/**
 * Hiring Module Database Setup Script
 *
 * Interactive Usage (prompts for input):
 *   node modules/hiring/scripts/db-setup.js
 *
 * Non-Interactive Usage (for automation/testing):
 *   node modules/hiring/scripts/db-setup.js --stage local --action setup
 *   node modules/hiring/scripts/db-setup.js --stage local --action rollback
 *   node modules/hiring/scripts/db-setup.js -s local -a setup
 */
...
async function runSetup(stage, action) {
  const sqlFile = action === 'setup'
    ? path.join(__dirname, '../hiring-setup.sql')
    : path.join(__dirname, '../hiring-setup-rollback.sql');

  let pool = null;

  try {
    console.log(`\n=== Hiring Module DB ${action.toUpperCase()} ===\n`);
    ...
    console.log(`\n✓ Hiring module ${action} completed successfully!\n`);
...
async function interactive() {
  try {
    ...
    if (action === 'rollback') {
      const confirm = await question('\nThis will DROP all hiring tables. Are you sure? (yes/no): ');
...
async function main() {
  ...
  if (parsed.stage && parsed.action) {
    ...
    console.log('Usage:');
    console.log('  Interactive:     node modules/hiring/scripts/db-setup.js');
    console.log('  Non-Interactive: node modules/hiring/scripts/db-setup.js --stage local --action setup');
    console.log('                   node modules/hiring/scripts/db-setup.js -s local -a rollback');
    process.exit(1);
  }
}
