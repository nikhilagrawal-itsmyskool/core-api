/**
 * Leave Module Database Setup Script
 *
 * Interactive:      node modules/leave/scripts/db-setup.js
 * Non-interactive:  node modules/leave/scripts/db-setup.js --stage local --action setup
 *                   node modules/leave/scripts/db-setup.js -s local -a rollback
 */
...
async function runSetup(stage, action) {
  const sqlFile =
    action === "setup"
      ? path.join(__dirname, "../leave-setup.sql")
      : path.join(__dirname, "../leave-setup-rollback.sql");
  let pool = null;
  try {
    console.log(`\n=== Leave Module DB ${action.toUpperCase()} ===\n`);
    ...
    console.log(`\n✓ Leave module ${action} completed successfully!\n`);
  } catch (error) {
    console.error(`\n✗ Error: ${error.message}\n`);
    throw error;
  } finally {
...
async function interactive() {
  try {
    ...
    if (action === "rollback") {
      const confirm = await question("\nThis will DROP all leave tables. Are you sure? (yes/no): ");
...
async function main() {
  ...
  if (parsed.stage && parsed.action) await nonInteractive(parsed.stage.toLowerCase(), parsed.action.toLowerCase());
    ...
    console.log("Usage:\n  node modules/leave/scripts/db-setup.js\n  node modules/leave/scripts/db-setup.js --stage local --action setup");
    process.exit(1);
  }
}
