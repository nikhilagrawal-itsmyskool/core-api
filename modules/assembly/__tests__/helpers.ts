...
export async function cleanupPlan(planId: string): Promise<void> {
  ...
  if (weeks.length) {
    ...
    for (const t of ['assembly_roster_entry', 'assembly_roster_participant', 'assembly_roster_day', 'assembly_roster_reference', 'assembly_week_unlock', 'assembly_checklist_tick', 'assembly_checklist_signoff']) {
