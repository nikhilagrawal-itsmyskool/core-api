...
(async () => {
  ...
  // fee receipts only — transport is tracked separately and has no charges in this ledger
  const receipts = {}; (await pool.query(`select student_id, coalesce(sum(total_paid),0) t from fee_receipt where school_id=$1 and academic_year_id=$2 and status='active' and student_id is not null and (type is null or type <> 'transport') group by student_id`, P)).rows.forEach(r => receipts[r.student_id] = n(r.t));
  ...
  for (const c of charges) {
    ...
    // Ledger concession AMOUNTS are the source of truth (defs are unreliable in prior years —
    // percentages stored as amounts, etc.), so we do NOT compare against def values. We only flag a
    // concession sitting on a head the student has no concession for (a real mis-attachment).
    const def = concByStuHead[c.student_id]?.[c.fee_head_id];
    s.expConc += def ? r2(def.value_type === 'percent' ? n(c.debit) * n(def.value) / 100 : Math.min(n(def.value), n(c.debit))) : 0;
    if (cc.concession > EPS && !def) s.flags.add('concession-on-wrong-head');
  ...
  console.log(`================ FEES LEDGER AUDIT (READ-ONLY) — DBPASN ${LABEL} ================`);
  ...
  const out = path.join(outDir, `fees-audit-${STAGE}-${LABEL}.csv`);
