/**
 * Library Module Database Setup Script
 *
 * Interactive Usage (prompts for input):
 *   node modules/library/scripts/db-setup.js
 *
 * Non-Interactive Usage (for automation/testing):
 *   node modules/library/scripts/db-setup.js --stage local --action setup
 *   node modules/library/scripts/db-setup.js --stage local --action rollback
 *   node modules/library/scripts/db-setup.js -s local -a setup
 */
...
async function runSetup(stage, action) {
  const sqlFile =
    action === "setup"
      ? path.join(__dirname, "../library-setup.sql")
      : path.join(__dirname, "../library-setup-rollback.sql");

  let pool = null;

  try {
    console.log(`\n=== Library Module DB ${action.toUpperCase()} ===\n`);
    ...
    console.log(`\n✓ Library module ${action} completed successfully!\n`);
  } catch (error) {
    console.error(`\n✗ Error: ${error.message}\n`);
    throw error;
  } finally {
  ...
  try {
    ...
    if (action === "rollback") {
      const confirm = await question(
        "\nThis will DROP all library tables. Are you sure? (yes/no): ",
      );
...
async function main() {
  ...
  if (parsed.stage && parsed.action) {
    ...
    console.log("Usage:");
    console.log("  Interactive:     node modules/library/scripts/db-setup.js");
    console.log(
      "  Non-Interactive: node modules/library/scripts/db-setup.js --stage local --action setup",
    );
    console.log(
      "                   node modules/library/scripts/db-setup.js -s local -a rollback",
    );
    process.exit(1);
  }
}
