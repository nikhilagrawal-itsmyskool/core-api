// DDC third summary - the 1000 Sections (3-digit whole numbers), sourced from
// the public "List of Dewey Decimal classes" (Wikipedia). This is the deepest
// level OCLC publishes freely. Deeper numbers (e.g. 796.358 cricket) are NOT
// here - they live in the paid DDC schedules and are handled via the curated
// list (library-ddc-curated) + the cataloguer's editable field + self-learning
// suggestions. "No longer used" / "Not assigned" cells are omitted.
//
// Level and parent are derived from the code at seed time:
//   X00 -> class (no parent), XY0 -> division (parent X00), else section (parent XY0).
...
732|Sculpture from earliest times to c. 500
...
744|Communication design and visual design
745|Decorative arts
746|Textile arts
...
755|Religion
757|Human figures
758|Nature, architectural subjects and cityscapes, other subjects
...
894|Literatures of Altaic, Uralic, Hyperborean, Dravidian languages
...
899|Literatures of non-Austronesian languages of Oceania and Austronesian languages
...
990|History of Australasia, Pacific Ocean islands, Atlantic Ocean islands, Arctic islands, Antarctica
`;

export const DDC_ALL: { code: string; label: string }[] = RAW.split("\n")
  .map((l) => l.trim())
  .filter(Boolean)
  .map((l) => {
    const i = l.indexOf("|");
    return { code: l.slice(0, i), label: l.slice(i + 1) };
  });
