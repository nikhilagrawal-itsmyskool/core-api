...
// throws — a notify failure must not fail the feedback action (delivery is async).
// This hits the free/instant in-app channel (no SMS/WhatsApp cost). Mirrors leave-notify.
  ...
  try {
    const res = await fetch(`${COMM_BASE_URL}/communication/notifications`, {
      ...
      headers: {
        ...
        Authorization: serviceAuthHeader({ name: "feedback" }),
      },
      body: JSON.stringify({
        recipientType,
        recipientIds: ids,
    ...
    if (!res.ok) {
      console.error(`[feedback] in-app notify failed: HTTP ${res.status}`);
    }
  } catch (err: any) {
    console.error(`[feedback] in-app notify error: ${err.message}`);
  }
}
