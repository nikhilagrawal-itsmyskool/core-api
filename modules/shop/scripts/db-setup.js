/**
 * Shop Module Database Setup Script
 *
 * Interactive Usage (prompts for input):
 *   node modules/shop/scripts/db-setup.js
 *
 * Non-Interactive Usage (for automation/testing):
 *   node modules/shop/scripts/db-setup.js --stage local --action setup
 *   node modules/shop/scripts/db-setup.js --stage local --action rollback
 *   node modules/shop/scripts/db-setup.js -s local -a setup
 */
...
async function runSetup(stage, action) {
  const sqlFile = action === 'setup'
    ? path.join(__dirname, '../shop-setup.sql')
    : path.join(__dirname, '../shop-setup-rollback.sql');

  let pool = null;
  try {
    console.log(`\n=== Shop Module DB ${action.toUpperCase()} ===\n`);
    ...
    console.log(`\n✓ Shop module ${action} completed successfully!\n`);
  } catch (error) {
    console.error(`\n✗ Error: ${error.message}\n`);
    throw error;
  } finally {
...
async function interactive() {
  try {
    ...
    if (action === 'rollback') {
      const confirm = await question('\nThis will DROP all shop tables. Are you sure? (yes/no): ');
...
async function main() {
  ...
  if (parsed.stage && parsed.action) {
    ...
    console.log('Usage:');
    console.log('  Interactive:     node modules/shop/scripts/db-setup.js');
    console.log('  Non-Interactive: node modules/shop/scripts/db-setup.js --stage local --action setup');
    console.log('                   node modules/shop/scripts/db-setup.js -s local -a rollback');
    process.exit(1);
  }
}
