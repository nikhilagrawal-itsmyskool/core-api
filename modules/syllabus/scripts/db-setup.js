/**
 * Syllabus Module Database Setup Script
 *
 * Interactive Usage (prompts for input):
 *   node modules/syllabus/scripts/db-setup.js
 *
 * Non-Interactive Usage (for automation/testing):
 *   node modules/syllabus/scripts/db-setup.js --stage local --action setup
 *   node modules/syllabus/scripts/db-setup.js --stage local --action rollback
 *   node modules/syllabus/scripts/db-setup.js -s local -a setup
 */
...
async function runSetup(stage, action) {
  const sqlFile =
    action === "setup"
      ? path.join(__dirname, "../syllabus-setup.sql")
      : path.join(__dirname, "../syllabus-setup-rollback.sql");

  let pool = null;

  try {
    console.log(`\n=== Syllabus Module DB ${action.toUpperCase()} ===\n`);
    ...
    console.log(`\n✓ Syllabus module ${action} completed successfully!\n`);
  } catch (error) {
    console.error(`\n✗ Error: ${error.message}\n`);
    throw error;
  } finally {
...
async function interactive() {
  try {
    console.log("\n=== Syllabus Module Database Setup ===\n");

    const stageInput = await question(
      `Select stage (${VALID_STAGES.join("/")}) [${DEFAULT_STAGE}]: `,
    );
    ...
    if (actionChoice === "1" || actionChoice.toLowerCase() === "setup") {
      ...
      actionChoice === "2" ||
      actionChoice.toLowerCase() === "rollback"
    ) {
      action = "rollback";
    } else {
    ...
    if (action === "rollback") {
      const confirm = await question(
        "\nThis will DROP all syllabus tables. Are you sure? (yes/no): ",
      );
...
async function main() {
  ...
  if (parsed.stage && parsed.action) {
    ...
    console.log("Usage:");
    console.log("  Interactive:     node modules/syllabus/scripts/db-setup.js");
    console.log(
      "  Non-Interactive: node modules/syllabus/scripts/db-setup.js --stage local --action setup",
    );
    console.log(
      "                   node modules/syllabus/scripts/db-setup.js -s local -a rollback",
    );
    process.exit(1);
  }
}
