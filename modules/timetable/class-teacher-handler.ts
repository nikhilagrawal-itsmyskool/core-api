...
import { CreateClassTeacherRequest, UpdateClassTeacherRequest } from './timetable-interfaces';
...
class ClassTeacherHandler {
  public list = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      const classTeachers = await classTeacherService.list(ctx.schoolId, qp.classId, qp.academicYearId);
  ...
  public getById = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      if (!result) { ResponseBuilder.notFound(ErrorCode.InvalidId, 'Class teacher not found', callback); return; }
  ...
  public create = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      if (!body.academicYearId || !body.classId || !body.teacherId) {
        ResponseBuilder.badRequest(ErrorCode.InvalidInput, 'academicYearId, classId and teacherId are required', callback); return;
      }
      ...
      if (body.firstPeriodSubjectId && !(await timetableService.subjectExists(body.firstPeriodSubjectId, ctx.schoolId))) {
        ResponseBuilder.badRequest(ErrorCode.InvalidInput, 'Invalid firstPeriodSubjectId', callback); return;
      }
      const result = await classTeacherService.create(body, ctx.schoolId, ctx.userId);
      ResponseBuilder.ok(result, callback);
    } catch (err: any) {
      ResponseBuilder.handleError(err, callback);
    }
  ...
  public update = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      const result = await classTeacherService.update(id, body, ctx.schoolId, ctx.userId);
      if (!result) { ResponseBuilder.notFound(ErrorCode.InvalidId, 'Class teacher not found', callback); return; }
      ResponseBuilder.ok(result, callback);
    } catch (err: any) {
      ResponseBuilder.handleError(err, callback);
  ...
  public remove = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      if (!existing) { ResponseBuilder.notFound(ErrorCode.InvalidId, 'Class teacher not found', callback); return; }
      await classTeacherService.delete(id, ctx.schoolId, ctx.userId);
      ResponseBuilder.ok({ message: 'Class teacher deleted successfully' }, callback);
    } catch (err: any) {
      ResponseBuilder.handleError(err, callback);
    }
  };
