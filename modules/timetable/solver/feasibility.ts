...
import { getDay, firstTeachingSlot, startPositions, teachingSlotCountByDay, totalTeachingSlots } from './grid';
...
export function checkFeasibility(input: SolverInput): FeasibilityReport {
  ...
  for (const lesson of input.lessons) {
    classDemand.set(lesson.classId, (classDemand.get(lesson.classId) || 0) + lesson.size);
    for (const t of lesson.teacherIds) {
      teacherDemand.set(t, (teacherDemand.get(t) || 0) + lesson.size);
    }
    if (lesson.size > maxRun) {
      issues.push(`A ${lesson.size}-period block for class ${lesson.classId} cannot fit: no day has ${lesson.size} consecutive teaching slots (longest run is ${maxRun}).`);
    }
  }

  for (const [classId, demand] of classDemand) {
    if (demand > totalSlots) {
      issues.push(`Class ${classId} needs ${demand} periods but only ${totalSlots} teaching slots exist in the week.`);
    }
  }
  ...
  for (const [teacherId, demand] of teacherDemand) {
    ...
    for (const c of hardByTeacher.get(teacherId) || []) {
      if (c.type === 'day_off') available -= perDay.get(c.value?.day) || 0;
        ...
        if (day?.slots.some((s) => s.slotType === 'teaching' && s.sequence === c.value?.slot)) available -= 1;
      } else if (c.type === 'weekly_max') weeklyCap = Math.min(weeklyCap, c.value?.max ?? Infinity);
    }
    const effective = Math.min(available, weeklyCap);
    if (demand > effective) {
      issues.push(`Teacher ${teacherId} is assigned ${demand} periods but is only free for ${effective} after their hard constraints.`);
    }
  }
  ...
  for (const lesson of input.lessons) {
    ...
    if (!day || !firstTeachingSlot(day)) {
      issues.push(`Class ${lesson.classId} has a class-teacher pin on day ${lesson.pinnedDay} but that day has no teaching slot.`);
    }
  }

  if (input.lessons.length === 0) warnings.push('No lessons to schedule — nothing was configured.');

  return { feasible: issues.length === 0, issues: [...new Set(issues)], warnings };
}
