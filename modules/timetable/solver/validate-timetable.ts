...
export function validateTimetable(
  ...
  for (const p of placements) {
    ...
    if (!lesson) {
      issues.push({
        rule: "unknown_lesson",
        message: `placement references unknown lesson ${p.lessonId}`,
      });
      continue;
    }
    if (p.slotIds.length !== lesson.size) {
      issues.push({
        rule: "size_mismatch",
        message: `lesson ${p.lessonId} needs ${lesson.size} slots, got ${p.slotIds.length}`,
      });
    }
    ...
    if (!day) {
      issues.push({
        rule: "invalid_day",
        message: `lesson ${p.lessonId} placed on non-existent day ${p.dayOfWeek}`,
      });
      continue;
    }
    const teachingByseq = new Map(
      day.slots
        .filter((s) => s.slotType === "teaching")
        .map((s) => [s.sequence, s.slotId]),
    );
    for (let k = 0; k < p.slotIds.length; k++) {
      ...
      if (!expected)
        issues.push({
          rule: "non_teaching_slot",
          message: `lesson ${p.lessonId} occupies a non-teaching/missing slot at day ${p.dayOfWeek} seq ${seq}`,
        });
      else if (expected !== p.slotIds[k])
        issues.push({
          rule: "slot_mismatch",
          message: `lesson ${p.lessonId} slot id mismatch at day ${p.dayOfWeek} seq ${seq}`,
        });
    }
  }
  ...
  for (const o of expandOccupancy(placements)) {
    ...
    if (teacherSlot.has(key)) {
      issues.push({
        rule: "teacher_double_book",
        message: `teacher ${o.teacherId} double-booked at day ${o.day} seq ${o.sequence}`,
      });
    } else {
  ...
  for (const p of placements) {
    ...
    if (
      !first ||
      p.dayOfWeek !== lesson.pinnedDay ||
      p.startSequence !== first.sequence
    ) {
      issues.push({
        rule: "class_teacher_pin",
        message: `class-teacher pin ${p.lessonId} is not at the first teaching slot of day ${lesson.pinnedDay}`,
      });
    }
  }

  // 5. Block rules: maxPeriodsPerDay (hard period cap) + legacy maxPerDay / notTwiceSameDay per group.
  const groupDayCount = new Map<string, number>();
  const groupDayPeriods = new Map<string, number>();
  for (const p of placements) {
    const lesson = lessonsById.get(p.lessonId);
    if (!lesson?.groupKey) continue;
    const key = `${lesson.groupKey}|${p.dayOfWeek}`;
    groupDayCount.set(key, (groupDayCount.get(key) || 0) + 1);
    groupDayPeriods.set(key, (groupDayPeriods.get(key) || 0) + p.size);
  }
  // Hard: periods/day per group must not exceed maxPeriodsPerDay.
  const capByGroup = new Map<string, number>();
  for (const lesson of input.lessons) {
    if (
      lesson.groupKey &&
      lesson.maxPeriodsPerDay !== undefined &&
      !capByGroup.has(lesson.groupKey)
    ) {
      capByGroup.set(lesson.groupKey, lesson.maxPeriodsPerDay);
    }
  }
  for (const [key, periods] of groupDayPeriods) {
    const [groupKey, day] = key.split("|");
    const cap = capByGroup.get(groupKey);
    if (cap !== undefined && periods > cap) {
      issues.push({
        rule: "max_periods_per_day",
        message: `group ${groupKey} has ${periods} periods on day ${day} (max ${cap})`,
      });
    }
  }
  for (const lesson of input.lessons) {
    ...
    for (const p of placements) {
      ...
      if (lesson.notTwiceSameDay && count > 1) {
        issues.push({
          rule: "not_twice_same_day",
          message: `group ${lesson.groupKey} appears ${count} times on day ${p.dayOfWeek}`,
        });
      }
      if (lesson.maxPerDay !== undefined && count > lesson.maxPerDay) {
        issues.push({
          rule: "max_per_day",
          message: `group ${lesson.groupKey} exceeds maxPerDay ${lesson.maxPerDay} on day ${p.dayOfWeek} (${count})`,
        });
      }
      break;
    }
  ...
  if (requireComplete) {
    ...
    for (const lesson of input.lessons) {
      if (!placedIds.has(lesson.id))
        issues.push({
          rule: "unplaced_lesson",
          message: `lesson ${lesson.id} (${lesson.groupKey || ""}) was not placed`,
        });
    }
    ...
    for (const classId of input.classIds) {
      for (const day of input.grid.days) {
        ...
        if (!first) continue;
        const occupant = classSlot.get(
          `${classId}|${day.dayOfWeek}|${first.sequence}`,
        );
        const lesson = occupant ? lessonsById.get(occupant) : undefined;
        if (
          !lesson ||
          lesson.pinnedDay !== day.dayOfWeek ||
          lesson.classId !== classId
        ) {
          issues.push({
            rule: "first_period_not_class_teacher",
            message: `class ${classId} day ${day.dayOfWeek} first period is not the class teacher's`,
          });
        }
      }
    }
