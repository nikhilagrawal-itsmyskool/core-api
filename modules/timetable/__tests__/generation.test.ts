...
async function setupSolvableConfig() {
  ...
  let res = await fetch(`${BASE_URL}/configs`, {
    method: "POST",
    headers,
    body: JSON.stringify({ academicYearId, name: "Gen Test Grid" }),
  });
  ...
  for (const dow of [1, 2, 3, 4, 5]) {
    res = await fetch(`${BASE_URL}/configs/${config.uuid}/days`, {
      method: "POST",
      headers,
      body: JSON.stringify({ dayOfWeek: dow }),
    });
  ...
  for (const [subjectId, teacherId] of [
    [eng.uuid, TEST_TEACHER_ID],
    [math.uuid, TEST_TEACHER_ID_2],
  ] as const) {
    ...
    await fetch(`${BASE_URL}/teaching-assignments`, {
      method: "POST",
      headers,
      body: JSON.stringify({ academicYearId, classId, subjectId, teacherId }),
    });
  }
  ...
  return {
    classId,
    academicYearId,
    configId: config.uuid as string,
    engId: eng.uuid as string,
  };
}
...
describe("Timetable API — generation flow", () => {
  it("feasibility → generate → worker → candidates → publish, with class-teacher first periods", async () => {
    ...
    // lock the grid — generation requires a locked (frozen) config
    ...
    res = await fetch(`${BASE_URL}/generate`, {
      method: "POST",
      headers,
      body: JSON.stringify({ configId, numCandidates: 2 }),
    });
    expect(res.status).toBe(200);
    ...
    for (const day of cfg.days)
      for (const slot of day.slots) seqOfSlot.set(slot.uuid, slot.sequence);

    for (const dow of [1, 2, 3, 4, 5]) {
      const firstPeriod = candidates[0].entries.find(
        (e: any) => e.dayOfWeek === dow && seqOfSlot.get(e.timeSlotId) === 1,
      );
    ...
    res = await fetch(
      `${BASE_URL}/published?academicYearId=${academicYearId}`,
      { headers },
    );
    expect(res.status).toBe(200);
  ...
  it("feasibility reports actionable issues for an over-constrained config", async () => {
    ...
    let res = await fetch(`${BASE_URL}/configs`, {
      method: "POST",
      headers,
      body: JSON.stringify({ academicYearId, name: "Tiny" }),
    });
    const config = await res.json();
    res = await fetch(`${BASE_URL}/configs/${config.uuid}/days`, {
      method: "POST",
    ...
    res = await fetch(`${BASE_URL}/feasibility`, {
      method: "POST",
      headers,
      body: JSON.stringify({ configId: config.uuid }),
    });
    const feas = await res.json();
