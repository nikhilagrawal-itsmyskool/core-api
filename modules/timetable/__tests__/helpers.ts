...
export async function getClassId(): Promise<string> {
  ...
  if (res.rows.length === 0)
    throw new Error(
      `No class found for school ${TEST_SCHOOL_CODE} — run sample-school-setup`,
    );
  cachedClassId = res.rows[0].uuid;
  return cachedClassId;
}

// Resolve up to `n` distinct class uuids for the test school.
export async function getClassIds(n: number): Promise<string[]> {
  const res = await getPool().query(
    `select c.uuid from class c
     join school s on s.uuid = c.school_id
     where lower(s.code) = lower($1) order by c.uuid limit $2`,
    [TEST_SCHOOL_CODE, n],
  );
  if (res.rows.length < n)
    throw new Error(
      `Need ${n} classes for school ${TEST_SCHOOL_CODE} — run sample-school-setup`,
    );
  return res.rows.map((r) => r.uuid);
}
...
export async function createSubject(
  kind = "academic",
): Promise<{ uuid: string; code: string }> {
  ...
  if (res.status !== 200)
    throw new Error(
      `createSubject failed (${res.status}): ${await res.text()}`,
    );
