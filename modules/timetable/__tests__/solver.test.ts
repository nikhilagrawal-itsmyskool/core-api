import {
  solve, validateTimetable, checkFeasibility, buildLessons, scoreTimetable,
  SolverGrid, SolverInput, Lesson,
} from '../solver';
  ...
  return {
    days: days.map((dayOfWeek) => ({
      ...
      slots: Array.from({ length: periods }, (_, i) => ({
        slotId: `d${dayOfWeek}s${i + 1}`, sequence: i + 1, slotType: 'teaching' as const,
      })),
    })),
  };
}
...
describe('solver — build-lessons', () => {
  ...
  it('expands doubles from block rules', () => {
    const { lessons } = buildLessons({
      classIds: ['C1'], teachingDays: [1, 2, 3, 4, 5],
      classSubjects: [{ classId: 'C1', subjectId: 'PHY', periodsPerWeek: 10, blockRules: { blocks: [{ size: 2, count: 2 }, { size: 1, count: 6 }] } }],
      teachingAssignments: [{ classId: 'C1', subjectId: 'PHY', teacherId: 'T2' }],
      classTeachers: [],
      electiveBands: [],
    });
  ...
  it('builds co-scheduled band lessons carrying every offering', () => {
    const { lessons } = buildLessons({
      classIds: ['C1'], teachingDays: [1, 2, 3],
      classSubjects: [], teachingAssignments: [], classTeachers: [],
      electiveBands: [{ bandId: 'B1', classId: 'C1', periodsPerWeek: 3, offerings: [{ subjectId: 'PHY', teacherId: 'T1' }, { subjectId: 'BIO', teacherId: 'T2' }] }],
    });
...
describe('solver — no-teacher subjects (a)', () => {
  ...
  it('does not treat two different teacher-less periods as a teacher clash', () => {
    ...
    const { lessons } = buildLessons({
      classIds: ['C1', 'C2'], teachingDays: [1],
      classSubjects: [
        { classId: 'C1', subjectId: 'STUDY', periodsPerWeek: 1 },
        { classId: 'C2', subjectId: 'STUDY', periodsPerWeek: 1 },
      ],
      teachingAssignments: [], classTeachers: [], electiveBands: [],
    });
    const input: SolverInput = { classIds: ['C1', 'C2'], grid, lessons, constraints: [], seed: 1 };
    const result = solve(input, 1);
    expect(result.feasible).toBe(true); // both fit the same slot — no phantom teacher clash
    expect(validateTimetable(input, result.candidates[0].timetable, false)).toEqual([]);
  });
});
...
describe('solver — admin-selectable first-period subject (c)', () => {
  ...
  it('auto-picks the class teacher subject with the most periods/week', () => {
    const { lessons } = buildLessons({ ...base, classTeachers: [{ classId: 'C1', teacherId: 'T1' }] });
    const pins = lessons.filter((l) => l.pinnedDay);
  ...
  it('honors an explicit firstPeriodSubjectId over the auto pick', () => {
    const { lessons } = buildLessons({ ...base, classTeachers: [{ classId: 'C1', teacherId: 'T1', firstPeriodSubjectId: 'LIB' }] });
    const pins = lessons.filter((l) => l.pinnedDay);
  ...
  it('falls back to auto (with a warning) when the chosen subject is not taught by the class teacher', () => {
    const { lessons, warnings } = buildLessons({ ...base, classTeachers: [{ classId: 'C1', teacherId: 'T1', firstPeriodSubjectId: 'NOPE' }] });
    const pins = lessons.filter((l) => l.pinnedDay);
    expect(pins.every((l) => l.offerings[0].subjectId === 'ENG')).toBe(true);
...
describe('solver — solve produces valid timetables', () => {
  function buildInput(): SolverInput {
    ...
    const { lessons } = buildLessons({
      ...
      classSubjects: [
        ...
        { classId: 'C1', subjectId: 'PHY', periodsPerWeek: 6, blockRules: { blocks: [{ size: 2, count: 1 }, { size: 1, count: 4 }] } },
      ],
      teachingAssignments: [
        { classId: 'C1', subjectId: 'ENG', teacherId: 'T1' },
  ...
  it('pins the class teacher into the first period of every teaching day', () => {
    ...
    for (const day of [1, 2, 3, 4, 5]) {
      const first = tt.placements.find((p) => p.dayOfWeek === day && p.startSequence === 1 && p.classId === 'C1');
  ...
  it('never double-books a teacher shared across two classes', () => {
    ...
    const { lessons } = buildLessons({
      classIds: ['C1', 'C2'], teachingDays: [1, 2, 3, 4, 5],
      classSubjects: [
        { classId: 'C1', subjectId: 'MATH', periodsPerWeek: 6 },
    ...
    const input: SolverInput = { classIds: ['C1', 'C2'], grid, lessons, constraints: [], seed: 3 };
    const result = solve(input, 1);
    expect(result.feasible).toBe(true);
    expect(validateTimetable(input, result.candidates[0].timetable, false)).toEqual([]);
  });
...
describe('solver — infeasibility & constraints', () => {
  it('reports infeasible when demand exceeds capacity (clean reason, no hang)', () => {
    ...
    const input: SolverInput = {
      classIds: ['C1'], grid, constraints: [],
      lessons: Array.from({ length: 5 }, (_, i) => ({ id: `L${i}`, classId: 'C1', size: 1, offerings: [{ subjectId: 'X', teacherId: 'T1' }], teacherIds: ['T1'] })) as Lesson[],
    };
  ...
  it('honors a hard day_off constraint', () => {
    ...
    const { lessons } = buildLessons({
      ...
      teachingAssignments: [{ classId: 'C1', subjectId: 'MATH', teacherId: 'TM' }],
      classTeachers: [], electiveBands: [],
    });
    const input: SolverInput = {
      classIds: ['C1'], grid, lessons, seed: 5,
      constraints: [{ teacherId: 'TM', type: 'day_off', value: { day: 3 }, hardness: 'hard' }],
    };
    const result = solve(input, 1);
    expect(result.feasible).toBe(true);
    const tt = result.candidates[0].timetable;
  ...
  it('is deterministic for a given seed', () => {
    ...
    const input: SolverInput = { classIds: ['C1'], grid, lessons, constraints: [], seed: 42 };
    const a = solve(input, 1).candidates[0];
    const b = solve(input, 1).candidates[0];
    expect(scoreTimetable(input, a.timetable).score).toBe(scoreTimetable(input, b.timetable).score);
  });
});
...
describe('solver — validator catches a hand-broken timetable', () => {
  it('flags a teacher double-book', () => {
    ...
    const input: SolverInput = {
      classIds: ['C1', 'C2'], grid, constraints: [],
      lessons: [
        { id: 'L1', classId: 'C1', size: 1, offerings: [{ subjectId: 'X', teacherId: 'T1' }], teacherIds: ['T1'] },
        { id: 'L2', classId: 'C2', size: 1, offerings: [{ subjectId: 'X', teacherId: 'T1' }], teacherIds: ['T1'] },
      ],
    };
    const broken = {
      placements: [
        { lessonId: 'L1', classId: 'C1', dayOfWeek: 1, startSequence: 1, slotIds: ['d1s1'], offerings: [{ subjectId: 'X', teacherId: 'T1' }], size: 1 },
        { lessonId: 'L2', classId: 'C2', dayOfWeek: 1, startSequence: 1, slotIds: ['d1s1'], offerings: [{ subjectId: 'X', teacherId: 'T1' }], size: 1 },
      ],
    };
