...
class ConfigHandler {
    ...
    try {
      ...
      if (!body.academicYearId || !body.name || !body.name.trim()) {
        ResponseBuilder.badRequest(
          ...
          "academicYearId and name are required",
          callback,
        );
        return;
      }
  ...
  public updateConfig = async (
    ...
    try {
      ...
      if (
        ...
        !CONFIG_STATUS_VALUES.includes(body.status)
      ) {
        ResponseBuilder.badRequest(
          ErrorCode.InvalidInput,
  ...
  // Lock a config (makes it immutable + generation-eligible).
  public lock = async (
    event: ApiEvent,
    _context: ApiContext,
    callback: ApiCallback,
    ...
    try {
      ...
      const id = requireParam(event, "id", callback);
      if (!id) return;
      const config = await configService.lock(id, ctx.schoolId, ctx.userId);
      if (!config) {
        ResponseBuilder.notFound(
          ErrorCode.InvalidId,
          "Config not found",
  ...
  // Unlock a config (allowed only while it has not been used to generate).
  public unlock = async (
    event: ApiEvent,
    _context: ApiContext,
    callback: ApiCallback,
    ...
    try {
      ...
      const id = requireParam(event, "id", callback);
      if (!id) return;
      const config = await configService.unlock(id, ctx.schoolId, ctx.userId);
      if (!config) {
        ResponseBuilder.notFound(
          ErrorCode.InvalidId,
          "Config not found",
  ...
  // Clone a config (deep copy of days + slots) into a new draft.
  public clone = async (
    event: ApiEvent,
    _context: ApiContext,
    callback: ApiCallback,
    ...
    try {
      ...
      const id = requireParam(event, "id", callback);
      if (!id) return;
      // Body is optional (just an alternate name); don't require one.
      const body: CloneConfigRequest = event.body ? JSON.parse(event.body) : {};
      const config = await configService.clone(
        id,
        ctx.schoolId,
        ctx.userId,
        body.name,
  ...
  public createDay = async (
    ...
    try {
      ...
      if (
        ...
        ResponseBuilder.badRequest(
          ErrorCode.InvalidInput,
          "dayOfWeek must be an integer 1-7 (1=Mon)",
          callback,
        );
        return;
      }
      const day = await configService.createDay(
        configId,
        body,
        ctx.schoolId,
        ctx.userId,
  ...
  public updateDay = async (
    ...
    try {
      ...
      if (!body) return;
      const day = await configService.updateDay(
        dayId,
        body,
        ctx.schoolId,
        ctx.userId,
  ...
  public createSlot = async (
    ...
    try {
      ...
      if (!body) return;
      // sequence orders slots within a day; non-positive values are allowed so
      // assembly (-1) and registration / 0th period (0) can sit before period 1.
      ...
      const slot = await configService.createSlot(
        dayId,
        body,
        ctx.schoolId,
        ctx.userId,
  ...
  public updateSlot = async (
    ...
    try {
      ...
      if (!body) return;
      if (
        body.slotType !== undefined &&
        !SLOT_TYPE_VALUES.includes(body.slotType)
      ) {
        ResponseBuilder.badRequest(
          ErrorCode.InvalidInput,
          `slotType must be one of: ${SLOT_TYPE_VALUES.join(", ")}`,
      ...
      if (body.sequence !== undefined && !Number.isInteger(body.sequence)) {
        ResponseBuilder.badRequest(
          ErrorCode.InvalidInput,
          "sequence must be an integer",
          callback,
      ...
      const slot = await configService.updateSlot(
        slotId,
        body,
        ctx.schoolId,
        ctx.userId,
