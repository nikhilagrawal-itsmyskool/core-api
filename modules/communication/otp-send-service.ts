...
// Logical template key for the recovery OTP (matches the `message_template` row and
// the login_otp table / auth_recovery_audit naming).
const OTP_TEMPLATE_KEY = 'login_otp';
...
//
// Template: logical key 'login_otp', channel 'sms'. When no active template row exists
...
class OtpSendService {
  public async send(schoolId: string, toNumber: string, code: string): Promise<{ status: 'sent' | 'failed'; providerMessageId?: string; error?: string }> {
    ...
    const byChannel = await templateService.getActiveByKey(schoolId, OTP_TEMPLATE_KEY, language);
    const template = byChannel.get('sms');

    const provider = getProvider();
    // A real BSP (MSG91) needs a registered flow id; without an active 'login_otp'
    // template there is nothing to send, so fail fast with a clear message instead of
    // firing a malformed request. The stub needs no template (it just logs), so let it through.
    if (!template && provider.name !== 'stub') {
      return { status: 'failed', error: "No active 'login_otp' SMS template configured" };
    }
