...
import { uniformPurchaseService } from './uniform-purchase-service';
import { UploadBillRequest } from './uniform-interfaces';
...
class UniformPurchaseHandler {
  public create = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      for (const item of body.items) {
        ...
        if (!item.sizeId) { ResponseBuilder.badRequest(ErrorCode.InvalidInput, 'Each item requires sizeId', callback); return; }
      ...
      // Validate all itemId/sizeId exist
      for (const item of body.items) {
      ...
      const result = await uniformPurchaseService.createBatch(body, schoolId, userId);
  ...
  public list = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      const results = await uniformPurchaseService.listBatches(schoolId, {
  ...
  public getById = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      const result = await uniformPurchaseService.getBatch(id, schoolId);
  ...
  public remove = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      const userId = event.requestContext?.authorizer?.principalId || 'system';
      const deleted = await uniformPurchaseService.deleteBatch(id, schoolId, userId);
  ...
  public uploadBill = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      const userId = event.requestContext?.authorizer?.principalId || 'system';
      const result = await uniformPurchaseService.uploadBill(id, body, schoolId, userId);
      if (!result) { ResponseBuilder.notFound(ErrorCode.InvalidId, 'Purchase not found', callback); return; }
      ResponseBuilder.ok(result, callback);
    } catch (err: any) {
      ResponseBuilder.handleError(err, callback);
  ...
  public getBill = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      const result = await uniformPurchaseService.getBill(id, schoolId);
  ...
  public deleteBill = async (event: ApiEvent, _context: ApiContext, callback: ApiCallback) => {
    ...
    try {
      ...
      const userId = event.requestContext?.authorizer?.principalId || 'system';
      const deleted = await uniformPurchaseService.deleteBill(id, schoolId, userId);
      if (!deleted) { ResponseBuilder.notFound(ErrorCode.InvalidId, 'Purchase not found', callback); return; }
...
const handler = new UniformPurchaseHandler();
