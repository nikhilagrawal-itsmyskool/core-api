/**
 * Examination Module Database Setup Script
 *
 * Interactive:      node modules/examination/scripts/db-setup.js
 * Non-interactive:  node modules/examination/scripts/db-setup.js --stage local --action setup
 *                   node modules/examination/scripts/db-setup.js -s local -a rollback
 */
...
async function runSetup(stage, action) {
  const sqlFile =
    action === "setup"
      ? path.join(__dirname, "../examination-setup.sql")
      : path.join(__dirname, "../examination-setup-rollback.sql");
  let pool = null;
  try {
    console.log(`\n=== Examination Module DB ${action.toUpperCase()} ===\n`);
    ...
    console.log(`\n✓ Examination module ${action} completed successfully!\n`);
  } catch (error) {
    console.error(`\n✗ Error: ${error.message}\n`);
    throw error;
  } finally {
...
async function interactive() {
  try {
    console.log("\n=== Examination Module Database Setup ===\n");
    ...
    if (action === "rollback") {
      const confirm = await question("\nThis will DROP all examination tables. Are you sure? (yes/no): ");
...
async function main() {
  ...
  if (parsed.stage && parsed.action) await nonInteractive(parsed.stage.toLowerCase(), parsed.action.toLowerCase());
    ...
    console.log("Usage:\n  node modules/examination/scripts/db-setup.js\n  node modules/examination/scripts/db-setup.js --stage local --action setup");
    process.exit(1);
  }
}
