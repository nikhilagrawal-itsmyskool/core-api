...
// Shared cross-entity lookups (no FKs — validated in app code), mirroring the small
// helpers the academic-calendar/homework modules use so this module stays self-contained.
...
// latest-starting year. Defaults the exam's year when the caller omits it.
...
// Grade = the grade-label prefix of a class/section name ("I-A" -> "I", "VIII-B" ->
// "VIII", "Nursery" -> "Nursery"). The datesheet is per-grade, so all sections of a
// grade share one column. Split on the first hyphen; trim surrounding spaces.
export function gradeOf(className: string): string {
  if (!className) return "";
  const i = className.indexOf("-");
  return (i === -1 ? className : className.slice(0, i)).trim();
}
