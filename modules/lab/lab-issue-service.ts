...
import { LabIssueLog, CreateIssueLogRequest, UpdateIssueLogRequest, ReturnItemRequest } from './lab-interfaces';
import { DEFAULTS } from './lab-constants';
const { generateShortUuid } = require('../../shared/util/generate-uuid.js');

class LabIssueService {
  public async create(
    ...
    const insertQuery = singleLineString`
      insert into lab_issue_log
      (uuid, item_id, lab_id, issue_date, quantity, issue_type, issued_to, purpose, expected_return_date, returned, status, school_id, createdby_userid, created_at, issued_to_type, issued_to_id)
    ...
    return this.getById(uuid, schoolId) as Promise<LabIssueLog>;
  }
  ...
  public async getById(id: string, schoolId: string): Promise<LabIssueLog | null> {
    const query = `
      ...
      from lab_issue_log il
      left join lab_item i on il.item_id = i.uuid
  ...
  public async search(params: {
    ...
    if (params.labId) {
      query += ` and il.lab_id = $${paramIndex++}`;
...
export const labIssueService = new LabIssueService();
