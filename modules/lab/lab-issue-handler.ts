...
import { labIssueService } from './lab-issue-service';
import { labItemService } from './lab-item-service';
import { labService } from './lab-service';
import { CreateIssueLogRequest, UpdateIssueLogRequest, ReturnItemRequest } from './lab-interfaces';
import { ISSUE_TYPES, ISSUED_TO_TYPES, RETURN_CONDITIONS } from './lab-constants';
import { getDefaultStartDate, getDefaultEndDate, isValidDate } from '../../shared/util/datetime';

class LabIssueHandler {
  public create = async (
    event: ApiEvent,
    _context: ApiContext,
    ...
    try {
      ...
      const result = await labIssueService.create(body, schoolId, userId);
  ...
  public update = async (
    ...
    try {
      ...
      const result = await labIssueService.update(id, body, schoolId, userId);
      if (!result) {
  ...
  public remove = async (
    ...
    try {
      ...
      const deleted = await labIssueService.delete(id, schoolId, userId);
      if (!deleted) {
        ResponseBuilder.notFound(ErrorCode.InvalidId, 'Issue not found', callback);
        return;
      }
  ...
  public getById = async (
    ...
    try {
      ...
      const result = await labIssueService.getById(id, schoolId);
      if (!result) {
        ResponseBuilder.notFound(ErrorCode.InvalidId, 'Issue not found', callback);
        return;
      }
  ...
  public list = async (
    ...
    try {
      ...
      const results = await labIssueService.search({
        schoolId,
        labId,
        itemId,
        issueType,
  ...
  public returnItem = async (
    ...
    try {
      ...
      const result = await labIssueService.returnItem(id, body, schoolId, userId);
      if (!result) {
        ResponseBuilder.notFound(ErrorCode.InvalidId, 'Issue not found', callback);
        return;
      }
...
const handler = new LabIssueHandler();
