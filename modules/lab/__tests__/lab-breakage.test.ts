...
describe('Lab Breakage API', () => {
  ...
  beforeAll(async () => {
    ...
    const labResponse = await fetch(labsUrl, {
      method: 'POST',
      headers,
      body: JSON.stringify({ name: 'Test Lab for Breakages', type: 'chemistry' }),
    });
  ...
  describe('GET /lab/breakages', () => {
    it('should list breakages with date range', async () => {
      const response = await fetch(
        `${breakagesUrl}?labId=${testLabId}&startDate=2026-01-01&endDate=2026-12-31`,
        { method: 'GET', headers }
      );
  ...
  describe('DELETE /lab/breakages/{id}', () => {
