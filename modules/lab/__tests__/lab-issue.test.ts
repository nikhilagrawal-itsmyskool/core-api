...
describe('Lab Issue API', () => {
  ...
  beforeAll(async () => {
    ...
    const labResponse = await fetch(labsUrl, {
      method: 'POST',
      headers,
      body: JSON.stringify({ name: 'Test Lab for Issues', type: 'chemistry' }),
    });
  ...
  describe('GET /lab/issues', () => {
    it('should list issues with date range', async () => {
      const response = await fetch(
        `${issuesUrl}?labId=${testLabId}&startDate=2026-01-01&endDate=2026-12-31`,
        { method: 'GET', headers }
      );
  ...
  describe('PUT /lab/issues/{id}/return', () => {
