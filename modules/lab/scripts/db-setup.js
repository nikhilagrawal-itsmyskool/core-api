/**
 * Lab Module Database Setup Script
 *
 * Interactive Usage (prompts for input):
 *   node modules/lab/scripts/db-setup.js
 *
 * Non-Interactive Usage (for automation/testing):
 *   node modules/lab/scripts/db-setup.js --stage local --action setup
 *   node modules/lab/scripts/db-setup.js --stage local --action rollback
 *   node modules/lab/scripts/db-setup.js -s local -a setup
 */
...
async function runSetup(stage, action) {
  const sqlFile = action === 'setup'
    ? path.join(__dirname, '../lab-setup.sql')
    : path.join(__dirname, '../lab-setup-rollback.sql');

  let pool = null;

  try {
    console.log(`\n=== Lab Module DB ${action.toUpperCase()} ===\n`);
    ...
    console.log(`\n✓ Lab module ${action} completed successfully!\n`);
...
async function interactive() {
  try {
    ...
    if (action === 'rollback') {
      const confirm = await question('\nThis will DROP all lab tables. Are you sure? (yes/no): ');
...
async function main() {
  ...
  if (parsed.stage && parsed.action) {
    ...
    console.log('Usage:');
    console.log('  Interactive:     node modules/lab/scripts/db-setup.js');
    console.log('  Non-Interactive: node modules/lab/scripts/db-setup.js --stage local --action setup');
    console.log('                   node modules/lab/scripts/db-setup.js -s local -a rollback');
    process.exit(1);
  }
}
