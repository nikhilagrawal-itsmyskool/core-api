...
const { loadModules, getGatewayPort } = require('./module-loader');
const { waitForHealth } = require('./health-module');
...
function parseArgs() {
  ...
  for (var i = 0; i < args.length; i++) {
    if (args[i] === '--stage' && args[i + 1]) {
      ...
      moduleList = args[i + 1].split(',').map(function(m) { return m.trim(); });
      i++;
    }
  }

  var modules = loadModules(stage);

  return {
    stage: stage,
    modules: moduleList || Object.keys(modules),
  };
}
...
function startModule(name, config, stage) {
  return new Promise(function(resolve, reject) {
    ...
    waitForHealth(name, config.httpPort, config.prefix, 60)
      .then(function() {
...
function startGateway(stage) {
  return new Promise(function(resolve) {
    ...
    var gatewayPort = getGatewayPort(stage);

    var proc = spawn('node', [gatewayPath, '--stage', stage], {
    ...
    proc.stdout.on('data', function(data) {
      ...
      if (data.toString().indexOf('Gateway URL') !== -1) {
        resolve({ proc: proc, port: gatewayPort });
      }
    });
...
async function main() {
  ...
  var allModules = loadModules(stage);
  var gatewayPort = getGatewayPort(stage);
  ...
  console.log('║  Gateway Port: ' + gatewayPort.toString().padEnd(46) + '║');
