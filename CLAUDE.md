...
- **feedback/**: Home-visit feedback / complaints — an **open-routing ticketing system**. A teacher records feedback for a student on a home visit (with optional evidence PDFs/images) and assigns it to a teacher; the ticket then flows freely — the **current owner or god** can reassign it to anyone (forward / send back) with a **mandatory comment (god exempt)**; any participant adds status-neutral **comments** with **@mentions + attachments**; the director (**god for now**) **completes / cancels** it (or **reopens** a terminal one). Lifecycle is just `open → completed | cancelled` (+ reopen); "awaiting director" is derived (open AND owner holds a reviewer role), not a stored status. Every act is one row in the append-only **`feedback_event`** timeline; files hang off events in the shared `file_storage` (`entity_type='feedback'`, `entity_id`=event uuid); **`feedback_watcher`** (recorder + every assignee + @mentioned; `last_seen_at` drives unread, `muted` suppresses pings) is the in-app notification audience. Header (`feedback`) keeps the ticket's current status/owner/student/category + `closed_by`/`closed_at`/`last_activity_at`. Actions: `feedback.view/record/respond` → teacher (respond = the `/me` surface: read own/watched/recorded, comment, forward/send back — owner-only), `feedback.review` → god (dashboard, list/summary, comment/assign on any ticket, complete/cancel/reopen) (guard() + feedback-actions.ts). Admin portal: Record (multi-card + evidence), My Feedback (To act/Watching/Recorded + unread), Dashboard (stat tiles + teacher breakup + filters), and a shared **Ticket Thread** (`/feedback/t/:id`, header + timeline + composer, both roles/PWA); NotificationBell deep-links via `entityType/entityId`. Migration: `scripts/migrate-to-ticketing.js` backfills the old flat model into the timeline.
